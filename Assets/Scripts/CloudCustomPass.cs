...
class CloudCustomPass : CustomPass
{
    ...
    protected override void Execute(CustomPassContext ctx)
    {
        ...
        using (new ProfilingScope(ctx.cmd, new ProfilingSampler("CloudCustomPass - Horizontal Blur")))
        {
            ...
            ctx.propertyBlock.SetFloat("_Radius", blurRadius / 4.0f);
            
            HDUtils.DrawFullScreen(ctx.cmd, cloudMaterial, downSampleBlurBuffer, ctx.propertyBlock, shaderPassId: 1);
        }
        ...
        using (new ProfilingScope(ctx.cmd, new ProfilingSampler("CloudCustomPass - Vertical Blur")))
        {
            ...
            HDUtils.DrawFullScreen(ctx.cmd, cloudMaterial, downSampleCloudBuffer, ctx.propertyBlock, shaderPassId: 2);
        }
